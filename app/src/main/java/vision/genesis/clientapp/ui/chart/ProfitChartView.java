...
//			lineEntries.add(new Entry(index, Math.abs(chart.getValue().floatValue())));
		...
		for (ChartSimple bar : pnLChart) {
			barEntries.add(new BarEntry(bar.getDate().getMillis(), bar.getValue().floatValue()));
...
//		chart.getAxisLeft().setAxisMinimum(min);
	...
	private void updateXAxis(DateRange dateRange) {
		XAxis xAxis = chart.getXAxis();
		xAxis.setAxisMinimum(dateRange.getFrom().getMillis());
		xAxis.setAxisMaximum(dateRange.getTo().getMillis());
...
//		yAxis.addLimitLine(createLimitLine(0f));
	...
	private void setPeriodLines(List<PeriodDate> periods) {
		...
		for (PeriodDate period : periods) {
			xAxis.addLimitLine(createPeriodLine(period.getDateFrom().getMillis()));
			xAxis.addLimitLine(createPeriodLine(period.getDateTo().getMillis()));
		}
	}
