...
@InjectViewState
public class ManagerProfitPresenter extends MvpPresenter<ManagerProfitView> implements DateRangeBottomSheetFragment.OnDateRangeChangedListener
{
	@Inject
...
//		if (managerId != null && programsManager != null && baseCurrency != null) {
		if (managerId != null && programsManager != null) {
			if (chartDataSubscription != null)
				chartDataSubscription.unsubscribe();
			//TODO: calculate maxPointCount
			chartDataSubscription = programsManager.getProfitChart(managerId, chartDateRange, 10)
					.observeOn(AndroidSchedulers.mainThread())
