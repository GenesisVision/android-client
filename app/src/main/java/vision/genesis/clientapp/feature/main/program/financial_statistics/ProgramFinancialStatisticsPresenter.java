...
@InjectViewState
public class ProgramFinancialStatisticsPresenter extends MvpPresenter<ProgramFinancialStatisticsView> implements DateRangeBottomSheetFragment.OnDateRangeChangedListener
{
	...
	private void handleGetHistoryResponse(ProgramPeriodsViewModel response) {
		...
		EventBus.getDefault().post(new SetProgramDetailsFinancialStatisticsCountEvent(response.getTotal()));
	...
	@Subscribe
	public void onEventMainThread(ShowFinancialStatisticsDetailsEvent event) {
		getViewState().showDetails(event.getPeriod());
	}
